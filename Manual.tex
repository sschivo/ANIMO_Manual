...
\pgfdeclareimage[height=3ex]{copy-icon}{images/copy}
...
% It is possible to change the granularity of a group of nodes at the same time: once the nodes are selected,
% right click on one of them and choose \emph{ANIMO} $\rightarrow$ \emph{Change number of levels} from the menu.
% A dialog window will open, showing a slider that allows to select the new number of levels for all the selected nodes.
% In a similar way, it is possible to ``rescale'' a group of edges with the right-click menu item \emph{ANIMO} $\rightarrow$
% \emph{Rescale k value(s)}: see Subsection~\ref{subsec:param-settings} for details.
% % it allows to automatically multiply the \emph{k} values of all the selected interactions
% % by a given value. For example, to double the speed of a group of reactions, a scale value of \texttt{2} should be given.
