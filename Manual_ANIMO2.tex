...
Choose the latest 2.x version ({\bf 2.8.3}, on the left column),
...
Please note that Cytoscape version 3 is currently not supported.
% \begin{enumerate}
...
(please note: the ``\url{http://}'' is required), then confirm with \emph{OK}. A \emph{development}, less stable version
of ANIMO is also available: to try it, use the URL ``\url{http://fmt.cs.utwente.nl/tools/animo/dev/plugins.xml}'' instead of
the one given above.
...
\emph{Analysis},
select \emph{ANIMO v1.38} (if you chose for the \emph{development} version of ANIMO, the number will be 2.x).
...
\emph{Analysis} $\rightarrow$ \emph{ANIMO v1.38}. If no plug-in can be updated, a message stating \emph{No
